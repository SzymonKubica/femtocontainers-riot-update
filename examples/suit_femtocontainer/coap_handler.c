                            ...
                            void *ctx)
{
    ...
    printf("[BPF handler]: getting appropriate SUIT backend depending on the storage "
           "location id. \n");
    ...
    printf(
        "[BPF handler]: getting a pointer to the data stored in the SUIT location. \n");
    suit_storage_read_ptr(storage, &mem_region, &length);

    printf("[BPF handler]: Application bytecode:\n");
    for (size_t i = 0; i < length; i++) {
        printf("%02x", mem_region[i]);
        // Add a new line every 8x8 bits -> each eBPF instruction is 64 bits long.
        if (i % 8 == 7) {
            printf("\n");
        }
    }
